...
export class ComplaintDetailPage {
    ...
    this.service.post_rqst( {'complaints_id':id},'app_karigar/getComplaintbyId').subscribe(response =>
      {
        ...
        this.invoiceImage = response['complaintDetails']['invoiceImage'][0];            
        ...
        this.invoiceImage.file_name =  this.sanitizer.bypassSecurityTrustResourceUrl( this.service.url+'app/uploads/'+this.invoiceImage.file_name  );
  ...
  rating(star)
    ...
    viewComplaintInvoiceImage()
    {
      this.modalCtrl.create(ViewProfilePage, {"Image": this.invoiceImage.file_name}).present();
    }
